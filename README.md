...
A customizable, local-first Discord bot for role-playing, chatGPT, and moderastion, and utilities, powered by the Ollama framework.
...
*   **User Commands:** Includes a `/reset` command to wipe chat history and start fresh (per-user/per-channel).
...
*   **Activation:** The bot listens for its name (as configured in `bot.py`), replies, or direct `@mentions`.
...
*   **Memory Reset:** Use `/reset` in any channel or DM to clear the context for that specific conversation.
